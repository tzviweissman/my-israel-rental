...
/**
 * Cover-image picker modal — shows every image attached to a property and
 * lets the host promote any of them to the cover slot in one click.
 *
 * Hits ``POST /api/properties/{id}/cover`` which only reorders existing
 * URLs (it cannot add or delete), so the modal is safe to expose anywhere
 * the host can already see the property.
 */
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-end md:items-center justify-center" data-testid="cover-picker-modal">
      <div className="bg-white w-full md:max-w-2xl md:rounded-2xl shadow-2xl max-h-[92vh] flex flex-col">
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Choose cover photo</h2>
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {images.length === 0 ? (
            <div className="text-center text-gray-500 py-12 text-sm">This property has no photos yet.</div>
          ) : (
                    ...
                    {!isCover && (
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 flex items-center justify-center transition-colors">
                        <span className="opacity-0 group-hover:opacity-100 px-2 py-1 rounded-md bg-white text-[#1E6A6A] text-xs font-semibold flex items-center gap-1 shadow">
                          {busyUrl === url ? 'Saving…' : <><CheckCircle2 size={12} /> Set as cover</>}
                        </span>
                      </div>
                    )}
                  </button>
