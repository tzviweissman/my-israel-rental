...
- **2026-09-08 — phase 5 (O8).** Cutoffs per weekday (Orders tab → Cutoffs; shown in the business page's good-to-know band; the entry form warns past a cutoff but never refuses), a Friday range on the board, and standing orders ("Repeat every week" on a card; next week's copy is generated lazily whenever the board is read, once, under a unique index; pause / resume / stop from the Weekly panel). `test_store_orders_standing.py`.
- **2026-09-08 — revision (Tzvi's decisions, same day).** O5 and O6 rebuilt around accounts, and ordering opened to customers:
  - **Couriers are accounts.** The business invites by email (`POST /businesses/{id}/couriers/invite`); the courier accepts from a new **Deliveries** tab in their own dashboard (`/courier/me`, `/courier/invites/{biz}/accept`, `/courier/deliveries`). The first to accept becomes the business's default courier. No capability links, no texts: `utils/sms.py`, `CourierRunSheet.jsx` and the token routes are gone.
  - **Assignment is automatic.** Every delivery order - typed, pasted, ordered on the site, or generated from a standing order - goes to the default courier the moment it exists (`auto_assign`); the owner can move it on the card. The courier gets a dashboard badge, the bell, and one email.
  - **Customers order on the site.** Every store listing shows *Order now* and a per-product *Order*; `/order/:gigId` (`pages/OrderPage.jsx`) takes products with quantities, a free line, pickup or delivery, a date the cutoffs allow, one of the store's time windows for that weekday, a city the store serves, name, phone, optional email. Rules enforced server-side in `place_website_order`: store prices, in-stock only, cutoffs, windows, service areas, minimum order, delivery fee. No account needed; a signed-in customer gets a **My orders** tab (`/orders/mine`).
  - **Settings** (`/orders/settings`): pickup and delivery windows per weekday, delivery fee, minimum order, default courier, plus the cutoffs. Public form reads them via `/order-form/{gig_id}`; the business page's payload carries `order_settings`.
  - **Proof of delivery.** The courier's photo is required for *Delivered* and is shown on the status link, in My orders, and in the "Delivered" email to the customer. The business is told of new website orders and of every delivered or failed stop (bell + email).
  - Tests: `test_store_orders_couriers.py` (rewritten), `test_store_orders_website.py` (new).
- **Not built:** the Meta WhatsApp template (no longer wanted - no texts at all), and the assistant's use of cutoffs (the assistant itself is not built).
