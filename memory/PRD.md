...
- [x] **Chat Re-attach on Duplicate Resolution (2026-06-17)**: User reported that after using the admin duplicate resolver, the renter's chat for the deleted twin opens "Property not found". Fix both going-forward AND provides recovery for already-orphaned chats.
  - **Backend** (`routes/admin.py::resolve_duplicates`): before deleting the loser property docs, the resolver now `update_many`s the loser_ids → keeper_id across `messages`, `bookings`, `chat_nudges`, `admin_blocks`, `subleases.original_property_id`, and `liked_properties` (with a dedupe pass so a renter who liked both copies doesn't end up with two rows). Response now includes `reattached: {messages, bookings, likes, nudges, blocks, subleases}` per group.
  - **Recovery endpoint** (`routes/admin.py::admin_reattach_chats`): new `POST /admin/chats/reattach {from_property_id, to_property_id}` lets the admin manually re-point an orphan conversation onto the surviving twin (rejects if source still exists or target is missing). Surfaces orphan status via a new `property_missing: true` flag on both `/chat/conversations` and `/admin/chats` responses.
  - **Frontend** (`components/admin/ChatsTab.jsx`): orphan rows render an inline amber "Listing removed — re-attach to surviving listing" bar with paste-id input + Re-attach button. Top-of-page orphan-count banner mirrors the existing unresponsive-count banner.
  - **Test coverage**: new `tests/test_duplicate_reattach.py` (2 tests) seeds two duplicate property docs with attached message/booking/like via direct Mongo, runs the resolver via HTTP, and asserts every related row now points at the keeper. 29/29 admin + properties + import tests green.
