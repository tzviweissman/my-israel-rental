...
- [x] **Stricter Duplicate Detection + One-Click Re-Mirror (2026-06-17)**: User reported that the duplicate resolver was flagging distinct apartments in the same building as duplicates, and that even after re-importing the CSV many vacation rentals still had no photos.
  - **Stricter dedupe signature** (`utils/dedupe.py`): the dedupe key now includes `bedrooms` and `floor` on top of the existing (owner_id, normalized_address, rental_type). Distinct units in the same building (3BR top floor + 2BR ground floor at the same street address) no longer collide — a real-world problem in Jerusalem where owners commonly have multiple units per building. New `dedupe_signature()` helper plus `_norm_int()` (so `'2'` and `2` from CSVs hash the same; `None` doesn't act as a wildcard against concrete values). Threaded the new fields through every caller: `routes/properties.py` (create + update), `routes/admin_import.py` (bulk + AI commit), `routes/bulk_upload.py`, `routes/admin.py` (`/admin/duplicates` + resolver — group key now `<owner>|<addr>|<rt>|<bedrooms>|<floor>`).
  - **One-click re-mirror tool** (`routes/admin_import.py::admin_remirror_properties`): new `POST /admin/properties/remirror` admin endpoint scans every property and classifies each as `queued` (has source URLs → fire background mirror task), `already_cdn` (skip — don't pay for redundant uploads), or `no_images` (empty `images` array, listed in response so admin knows which need a CSV re-upload in "Sync photos" mode). Marks `mirror_pending: true` immediately so the UI can spinner without waiting for the background task. Returns 503 cleanly if Cloudinary isn't configured.
  - **Admin UI** (`components/admin/ListingsTab.jsx`): new sky-blue "Re-mirror photos" button next to "Find duplicates". Confirm prompt explains scope; success toast shows queued / already_cdn / no_images counts; a separate warning toast surfaces the `no_images` sample (titles/ids) so the admin knows which listings still need CSV upload.
  - **Test coverage**: new `tests/test_dedupe_signature.py` (7 tests — different bedrooms/floor → distinct, identical sig → duplicate, None vs concrete, string-numerics normalize, rental_type still bypasses, missing address yields None) and `tests/test_admin_remirror.py` (2 tests — classification correctness, auth required). 17/17 dedupe + import + remirror tests green.
