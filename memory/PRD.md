...
> **DISCONTINUED — (1) document / government "paid services" (Arnona discount, name change, document filing, Bituach Leumi benefits) and (2) storage rentals (the `storage` rental_type) are no longer offered.** Do not surface them in UI/marketing or build new work for them. Historical changelog entries below that reference document services are kept for audit only; treat the feature as retired. (The related code/feature flag `DOCUMENT_SERVICES_ENABLED` should stay off / be cleaned up separately.)

## Original Problem Statement
Build a bilingual (English/Hebrew) rental website named MyIsraelRental.com with admin dashboard, property listing management (long-term, short-term, vacation), real-time chat, iCal/Airbnb calendar sync, rental contract translation & signing, renter notifications, and manager bulk upload features. Plus a two-sided local **services marketplace** (hire/offer pros) and a **Requests board** (post what you're looking for). Blue and gold color scheme.
...
- [x] **Performance sweep — 5-item bundle-size & network optimization pass (2026-07-15)**:
  ...
  - **3. Emergent script `defer`** (`frontend/public/index.html`): added `defer` to `<script src="https://assets.emergent.sh/scripts/emergent-main.js">` so it no longer blocks HTML parsing. The inline error handler above it doesn't depend on the script, and PostHog was already deferred in `<body>` — this change makes the head consistent. Live verified: `document.querySelector('script[src*="emergent-main.js"]').defer === true`.
  - **4. Pinch-zoom lock removed** (`frontend/public/index.html`): viewport meta changed from `width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no, viewport-fit=cover` → `width=device-width, initial-scale=1, viewport-fit=cover`. Fixes **WCAG 2.1 SC 1.4.4 (Resize Text) + SC 1.4.10 (Reflow)** for low-vision users. The 16px min-font-size rule in `index.css:34` still prevents iOS input-focus auto-zoom, so the blanket lock was redundant.
  - **5. Services-hero videos shrunk**: re-encoded all 6 mp4 files with `ffmpeg -vf scale=-2:720 -c:v libx264 -crf 30 -preset veryfast -movflags +faststart -an`. **Result**: 25.2 MB → 1.28 MB total (**~95% reduction**). K-carpenter.mp4 alone: 7.2 MB → 337 kB. Note: these videos are no longer referenced by any component (background video was removed from Services hero in a prior iteration) — this is a static-asset cleanup that saves deployment bundle size and future page-weight if the videos are ever reintroduced.
  - **Testing**: iteration_64.json — **11/11 test areas PASS, 100% backend + frontend, zero issues found**. All prior iteration flows (back-navigation, MyJobsTab, job-chat integration, notification preferences) still work.
  - Files touched: `frontend/src/App.js`, `backend/server.py`, `frontend/public/index.html`, `frontend/public/videos/services-hero/*.mp4` (6 files).
...
- [x] **Job-scoped chat email notifications — "Job: <title>" subject fallback (2026-07-15)**:
  ...
  - The email template's deep-link is `${FRONTEND_URL}/chat/{property_id}?with={sender_id}` — since the frontend already falls back to the marketplace_jobs endpoint when `/properties/{id}` 404s (last iteration), the link opens the job-scoped chat thread with the correct job info bar (gold Briefcase icon, "JOB" pill) on first paint.
  - **Existing safeguards inherited automatically**: chat_email_throttle prevents 20-message bursts from generating 20 emails, `_recipient_is_actively_in_chat` suppresses notifications when the recipient is already reading, and the module-level `_bg_email_tasks` set keeps the fire-and-forget task from being garbage-collected mid-Postmark-call (email reliability fix from iteration 59).
  - **Regression tests**: `/app/backend/tests/test_chat_email_job_title.py` (3 unit tests, all pass). Covers (1) job resolves → `"Job: <title>"` subject, (2) property resolves → property title unchanged (no `Job:` prefix, jobs collection never queried), (3) neither resolves → historical `"your conversation"` fallback preserved. Live curl also confirmed the end-to-end path (Postmark got as far as its recipient-suppression 406 check, meaning our code reached the send call with the correct payload).
  - Files touched: `backend/routes/chat.py` (16 lines added), `backend/tests/test_chat_email_job_title.py` (new).


- [x] **Job-scoped chat threads — poster ↔ applicant messaging (2026-07-15)**:
  - Closed the last real friction in the Jobs Board reverse-marketplace loop. Previously the poster could see applications in `MyJobsTab` but had no way to respond in-platform — they had to reach out via WhatsApp/email on the provider's gig profile.
  - **New "Message" button** on every application row in `MyJobsTab.jsx` (`data-testid=applicant-message-btn-{id}`). Clicking it navigates to `/chat/{job_id}?with={applicant.provider.user_id}` — the same chat surface used by property inquiries, now scoped to a Jobs Board post. Button disables when `user_id` is null (defensive against enrichment gaps on deleted provider rows).
  - **`Chat.js` fetchProperty fallback**: when `GET /api/properties/{id}` returns 404, the chat page falls back to `GET /api/marketplace/jobs/{id}`. If the job resolves, its doc lands in a new `job` state which is passed down to `ChatHeader`. Counterparty resolution: prefers `?with=` override (poster→applicant), else falls back to `poster_user_id` (applicant→poster). Pure orphan ids (no property, no job) still work via the `?with=` counterparty override — no crash.
  - **`ChatHeader.jsx` new job info bar**: rendered when `!property && job`. Same visual shape as the property/sublease bar for consistency: gold Briefcase icon (differentiates from teal Building2 on property chats), job title, area, budget (or "Open to offers"), and a **"JOB" pill** on the right. Test-ids: `chat-header-job-bar`, `chat-header-job-title`.
  - **`/chat/conversations` backend fallback** (`routes/chat.py`): when the property lookup misses, the endpoint now also tries `db.marketplace_jobs.find_one({_id: property_id})`. Job threads are labelled `"Job: <title>"` in the inbox (visually distinguishes from property threads) and get a new `is_job_thread: true` flag on the response (`ConversationOut` has `extra='allow'` so no model change needed).
  - **How it works under the hood**: chat messages are stored in `db.messages` with `property_id = <job_uuid>`. Since job UUIDs never collide with property UUIDs (astronomical probability, different collections), threads are naturally isolated. The `property_id` field name is now semantically "thread_id" — noted in comments; a rename is deferred to avoid churn.
  - **Testing**: iteration_62.json — 10/10 backend integration tests + 8/8 frontend flows PASS. Regression test at `/app/backend/tests/test_job_chat_integration.py` covers: apply→chat message send→conversations 'Job:' labeling→two-way reply→property-chat non-regression→orphan uuid graceful degradation. Property chats still render the property bar (Building2 + rental_type pill), NOT the job bar — verified.
  - Files touched: `frontend/src/components/dashboard/MyJobsTab.jsx`, `frontend/src/pages/Chat.js`, `frontend/src/components/chat/ChatHeader.jsx`, `backend/routes/chat.py`, `backend/tests/test_job_chat_integration.py` (new).


- [x] **Jobs Board reverse marketplace — poster dashboard (`MyJobsTab`) + applicant enrichment fix (2026-07-15)**:
  - Completed the reverse-marketplace loop by adding the missing poster-side dashboard. The Jobs Board's backend (`routes/marketplace/jobs.py`, 602 lines: CRUD, applications, matches, saved-searches, digest emails, category-matched provider notifications) and the public discovery / provider surfaces (`JobsBoard.jsx`, `PostJob.jsx`, `JobDetail.jsx`, provider-side `JobRequestsTab.jsx`) were already built. The gap: `GET /marketplace/my-jobs` returned data but no UI consumed it — a customer who posted a job had no way to see their applicants.
  - **New `frontend/src/components/dashboard/MyJobsTab.jsx`** (~280 lines): expandable job cards showing title, description snippet, budget, area, status pill (open/awarded/closed), and application count badge. Clicking a card lazy-loads `GET /marketplace/jobs/{id}/applications` and inline-renders each applicant with name, response bucket, timestamp, message, and quoted price. Actions: **Close** (PATCH status:closed), **Reopen** (PATCH status:open), **View public page** (→ `/services/jobs/{id}`). Empty state links to `/services/post-job`. Test-ids: `my-jobs-tab`, `my-jobs-list`, `my-jobs-empty`, `my-job-card-{id}`, `my-job-toggle-{id}`, `my-job-app-count-{id}`, `my-job-status-{status}`, `my-job-view-{id}`, `my-job-close-{id}`, `my-job-reopen-{id}`, `applicant-{id}`, `applicant-name-{id}`, `applicant-quote-{id}`, `applicant-message-{id}`.
  - **Dashboard wiring**: new `tab-my-jobs` tab in `DashboardTabs.jsx`, ungated (visible to all roles — anyone can post a job, so anyone should see their history). Deep-link `/dashboard?tab=my-jobs` already supported by the existing tab-param handler. `Dashboard.js` routes `activeTab==='my-jobs'` to the new component.
  - **`PostJob.jsx` success nudge**: on successful post, the toast now has an action button "My jobs" (sonner action prop) that navigates to `/dashboard?tab=my-jobs` — closes the "I posted, now what?" loop.
  - **Backend bugfix in `_list_applications` enrichment** (`routes/marketplace/jobs.py:392-405`): the users lookup was querying `db.users.find({"_id": {"$in": provider_ids}})` where `_id` is a Mongo ObjectId, but `provider_user_id` on applications is a UUID stored in `users.id`. So the users-dict was always empty and every applicant's `display_name` fell through to the literal fallback string "Provider". Fixed by querying by `id` (UUID) and keying the dict on `id` — also switched the fallback field from the non-existent `full_name` to the real `name` field. Verified live: applicant now correctly shows "Test Owner" instead of "Provider".
  - **Regression tests locked in**: `/app/backend/tests/test_my_jobs.py` (Pytest, 6 cases). Testing agent verified via iteration_61.json — 100% pass on backend + all frontend flows (deep-link, list, count badge, expand, applicant enrichment, close→closed pill, reopen→open pill, view public, empty state, tab visibility for renter AND owner). Live curl also confirmed the enrichment fix returns "Test Owner" for an owner-user applicant with no `marketplace_providers` row.
  - Files touched: `frontend/src/components/dashboard/MyJobsTab.jsx` (new), `frontend/src/pages/Dashboard.js`, `frontend/src/components/dashboard/DashboardTabs.jsx`, `frontend/src/pages/PostJob.jsx`, `backend/routes/marketplace/jobs.py`, `backend/tests/test_my_jobs.py` (new).
  - **v2 backlog** (not in this scope): per-applicant "Award" button + backend endpoint that atomically marks the application as accepted and other applications as rejected, in-platform job-scoped chat threads between poster ↔ applicant, and applicant ratings after job completion.

- [x] **Back-navigation filter preservation + shared hook (2026-07-15)**:
  - Re-applied the fixes from branch `claude/back-button-filters-iqtoda` into `main` on preview, then extracted the near-verbatim duplicated `previousPath` save/restore code into a single shared hook at `frontend/src/hooks/useBackNavigation.js` — exports `saveReturnPath()` (writes `pathname + search` to `sessionStorage.previousPath` right before drilling into a detail page) and `useReturnDestination(prefixWhitelist, fallback)` (detail-page helper that returns the stored path if it starts with one of the whitelisted prefixes, otherwise a fallback URL).
  - **Bugs closed**: (1) `PropertyDetail.js` `isFromListings` only matched `/properties/*`, so `/stays` and `/kosher-stays-in-israel` renters always fell through to a bare `/stays` with filters wiped. Now also matches those routes. (2) `ManagerPage.js` filters (`type`, `area`, `bedrooms`) lived only in `useState`, so back-button was reset. Now mirrored into the URL via `useSearchParams` — same pattern as Properties.js/Stays.jsx — and saved on card-click. (3) Services marketplace back-buttons on `GigDetail.jsx`, `ProviderProfile.jsx`, and `JobDetail.jsx` all hard-navigated to bare `/services` or `/services/jobs`, ignoring the filter/context of origin. All 3 now use `useReturnDestination` so back returns to the exact filtered list — including cross-page flows like `Gig → View provider → Back returns to Gig`.
  - **Small cleanups**: `Properties.js:707` (holiday quick-filter pills) no longer saves pathname-only — now uses `saveReturnPath()` for consistency with the card-click save. `PostJob.jsx` back button now goes to `/services/jobs` instead of `/services` (matches the create-new flow's actual origin). Whitelist approach also mitigates stale-sessionStorage hijacks (e.g. a leftover `/dashboard` entry can't hijack `gig-back` — verified via test).
  - Files touched (10): `hooks/useBackNavigation.js` (new), `pages/Properties.js`, `pages/Stays.jsx`, `pages/ManagerPage.js`, `pages/PropertyDetail.js`, `pages/Services.jsx`, `pages/JobsBoard.jsx`, `pages/GigDetail.jsx`, `pages/ProviderProfile.jsx`, `pages/JobDetail.jsx`, `pages/PostJob.jsx`.
  - **Regression tests locked in**: `/app/tests/test_back_navigation.py` (Playwright, 9 flows). Verified end-to-end via testing agent (iteration_60.json — 9/9 pass) covering: /properties/, /stays, /manager/, /services, /services/jobs, cross-page gig↔provider, PostJob back destination, and stale-sessionStorage fallbacks for both gig-back and job-detail-back. Success rate: 100% frontend.
  - **Breadcrumb on detail pages (2026-07-15)**: added `frontend/src/components/common/Breadcrumb.jsx` — a filter-aware "you came from" trail that reads the same `sessionStorage.previousPath` as the back button. Renders as `[origin label] › [current page]` on `PropertyDetail`, `GigDetail`, `ProviderProfile`, and `JobDetail`. `parseOrigin()` inspects the stored URL and decorates the origin with the active filter chip: `/stays?area=Jerusalem` → "Stays · Jerusalem", `/properties/long-term?min_bedrooms=2` → "Long-term rentals · 2+ bed", `/services?category=home-repair` → "Services · Home repair", `/services/jobs?category=cleaning` → "Jobs board · Cleaning", `/manager/<id>?area=Jerusalem` → "Manager listings · Jerusalem". Clicking the origin link jumps back to the exact stored URL (so the trail *is* the back button, just labelled). If `previousPath` is empty or doesn't match a whitelisted prefix, the whole breadcrumb collapses to `null` — no broken/misleading trails ever render. Test-ids: `property-breadcrumb`, `gig-breadcrumb`, `provider-breadcrumb`, `job-breadcrumb` with `-origin` / `-current` sub-testids. Verified live via screenshot on PropertyDetail (`Stays · Jerusalem › ss`) — clean, small, above the existing Back button so both affordances coexist.
