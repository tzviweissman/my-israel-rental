...
class TestProperties:
    def test_list_properties_enriched(self):
        ...
        The PUBLIC list is a lean card projection (id, title, price, one
        cover image, area) - the payload win that made /stays fast, and a
        contract this used to contradict by demanding owner_id and
        amenities from it. The OWNER-SCOPED list (?owner_id=) returns the
        whole document, because the dashboard's edit form hydrates from
        it; that is where the rich shape is asserted."""
        r = requests.get(f"{API}/properties", timeout=30)
        assert r.status_code == 200
        props = r.json()
        assert isinstance(props, list) and len(props) > 0, "no properties returned"
        p = props[0]
        for k in ("id", "title", "rental_type", "property_type", "area"):
            assert k in p, f"public card missing {k}"
        assert len(p.get("images") or []) <= 1, "public list must ship one cover image, not the gallery"
        ...
        owned = requests.get(f"{API}/properties", params={"owner_id": me["id"]}, timeout=30).json()
        assert owned, "owner has no properties"
        full = owned[0]
        for k in ("id", "title", "owner_id", "rental_type", "property_type", "currency", "status"):
            assert k in full, f"owner-scoped property missing {k}"
        assert len(full.keys()) >= 15, f"owner-scoped property seems thin: {list(full.keys())}"
        _assert_no_mongo_id(owned)
...
class TestBookings:
    def test_create_booking_flow(self):
        ...
        start = date.today() + timedelta(days=400 + random.randint(0, 18000))
        payload = {
            "property_id": pid,
            # Computed and spread per run: a fixed window collided with the
            # booking the previous run made on the same property.
...
class TestBulkEndpoints:
    def test_bulk_edit_shape(self):
        ...
        me = requests.get(f"{API}/auth/me", headers=_hdr(owner_tok), timeout=10).json()
        # The public list is a lean card projection with no owner_id (a
        # deliberate payload win); the owner-scoped list returns the full
        # documents. Filtering the public list by owner_id matched nothing.
        owned = requests.get(f"{API}/properties", params={"owner_id": me["id"]}, timeout=20).json()
    ...
    def test_bulk_images_shape(self):
        owner_tok = _login(*OWNER)["token"]
        me = requests.get(f"{API}/auth/me", headers=_hdr(owner_tok), timeout=10).json()
        # Owner-scoped list: the public one is a lean card projection with
        # no owner_id, so filtering it by owner matched nothing.
        owned = requests.get(f"{API}/properties", params={"owner_id": me["id"]}, timeout=20).json()
        assert owned, "owner has no properties"
        pid = owned[0]["id"]
        r = requests.post(
